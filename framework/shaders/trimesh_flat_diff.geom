...
{		
	...
	float diff = diff_g[0] * 10f;
	float red = 0.2f + diff * 0.25f;
	red = min(red, 0.8f);
	float green = 0.2f + diff;
	green = min(green, 0.8f);
	green = green - (red - 0.2f);
	float blue = 0.8f - 0.8f * diff;
	blue = max(blue, 0.2f);
	color_g = vec4(red, green, blue,1.f);
